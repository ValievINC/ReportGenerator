...
# Filling
def fill_employees_page(workbook, data, date, date_stat):
    ...
    for employee in workbook.sheetnames[1:]:
        if employee.title() not in today_employees_list:
            ...
            if date_stat['holiday'] != 0 or date_stat['weekend'] != 0:
                sheet[f'{letter}4'].fill = yellow_fill
                sheet[f'{letter}4'].value = 'В'
            else:
                sheet[f'{letter}4'].fill = red_fill
        else:
            sheet = workbook[employee.title()]
            sheet[f'{letter}4'] = data.loc[data['Сотрудник'] == employee.title()]['Время, часы'].item()
            sheet[f'{letter}5'] = date_stat['working_hours']
...
def fill_summary_table(workbook, old_data):
    ...
    for employee_sheet in workbook.worksheets[1:]:
        ...
        for i in range(1, 31):
            ...
            working_hours = employee_sheet[f'{letter_for_employee}5'].value
            if time is not None and type(time) is not str:
                if i < 16:
                    sheet[f'{get_column_letter(i+4)}{pos}'] = 'Я'
                    sheet[f'{get_column_letter(i+4)}{pos + 1}'] = '' if time == working_hours else time - working_hours
                else:
                    sheet[f'{get_column_letter(i-11)}{pos + 2}'] = 'Я'
                    sheet[f'{get_column_letter(i-11)}{pos + 3}'] = '' if time == working_hours else time - working_hours
